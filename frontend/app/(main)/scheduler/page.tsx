...
export default function SchedulerPage() {
    ...
    const updateEventList = (events: any[], updatedEvent: any) => {
        return events.map((event) => event.eventId === updatedEvent.eventId ? updatedEvent : event);
    };

    const syncCategorizedEvents = (previousEvents: any[], previousType: string, nextType: string, updatedEvent: any) => {
        if (previousType === updatedEvent.eventType) {
            return updateEventList(previousEvents, updatedEvent);
        }

        if (previousEvents.some((event) => event.eventId === updatedEvent.eventId)) {
            return previousEvents.filter((event) => event.eventId !== updatedEvent.eventId);
        }

        return nextType === updatedEvent.eventType ? [...previousEvents, updatedEvent] : previousEvents;
    };
    ...
    const handleUpdateEvent = async () => {
        try {
            ...
            setEvents((previousEvents) => updateEventList(previousEvents, updatedEvent));
            setClasses((previousEvents) => syncCategorizedEvents(previousEvents, selectedEvent.eventType, "CLASS", updatedEvent));
            setDeadlines((previousEvents) => syncCategorizedEvents(previousEvents, selectedEvent.eventType, "DEADLINE", updatedEvent));
            setExams((previousEvents) => syncCategorizedEvents(previousEvents, selectedEvent.eventType, "EXAM", updatedEvent));
            setOthers((previousEvents) => syncCategorizedEvents(previousEvents, selectedEvent.eventType, "OTHER", updatedEvent));
        } catch (error: any) {
    ...
    const handleDeleteEvent = async () => {
        try {
            ...
            setClasses(classes.filter((event) => event.eventId !== selectedEvent.eventId));
            setDeadlines(deadlines.filter((event) => event.eventId !== selectedEvent.eventId));
            setExams(exams.filter((event) => event.eventId !== selectedEvent.eventId));
            setOthers(others.filter((event) => event.eventId !== selectedEvent.eventId));
        } catch (error: any) {
