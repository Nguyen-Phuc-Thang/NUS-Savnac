...
import { getDate } from '@/lib/constants/nusCalendar';
...
export default function CoursePage() {
    ...
    const [folderGridMode, setFolderGridMode] = useState<
        'NORMAL' | 'EDIT' | 'DELETE'
    >('NORMAL');
    ...
    const [linkGridMode, setLinkGridMode] = useState<
        'NORMAL' | 'EDIT' | 'DELETE'
    >('NORMAL');
    ...
    const [eventReminderInput, setEventReminderInput] = useState(false);
    ...
    const getCourseData = async () => {
        try {
            const dataFromNUS = await getNUSCourseData(
                String(params.courseCode),
            );
            const dataFromDatabase = await getCourseInfo(
                String(params.courseCode),
                session?.user?.id || '',
            );
            const fullCourseData = {
                courseId: dataFromDatabase?.courseId,
                ...dataFromNUS,
            };
    ...
    const getAllFolders = async (courseId: string) => {
        try {
            ...
            setFolders(
                folders.filter((folder: any) => folder.name !== '__general__'),
            );
            setGeneralFolder(
                folders.find((folder: any) => folder.name === '__general__'),
            );
            setSelectedFolder(
                folders.find((folder: any) => folder.name === '__general__'),
            );
    ...
    const handleCreateFolder = async () => {
        try {
            ...
            toast.success(
                'Folder ' + folderNameInput + ' created successfully!',
            );
    ...
    const handleUpdateFolder = async () => {
        try {
            ...
            toast.success(
                'Folder ' + folderNameInput + ' updated successfully!',
            );
            setFolders(
                folders.map((folder) =>
    ...
    const handleDeleteFolder = async () => {
        try {
            ...
            setFolders(
                folders.filter(
                    (folder) => folder.folderId !== selectedFolder.folderId,
                ),
            );
            toast.success(
                'Folder ' + selectedFolder.name + ' deleted successfully!',
            );
        } catch (error: any) {
            toast.error(
                error.message ||
    ...
    const handleCreateLink = async () => {
        try {
            ...
            toast.error(
                error.message ||
                    'Failed to create link. Please try again later.',
            );
        }
    };
    ...
    const handleUpdateLink = async () => {
        try {
            ...
            const newFolder = {
                ...selectedFolder,
                links: selectedFolder.links.map((link: any) =>
                    link.linkId === selectedLink.linkId
                        ? { ...link, title: linkTitleInput, url: linkUrlInput }
                        : link,
                ),
            };
            setSelectedFolder(newFolder);
            if (selectedFolder.folderId === generalFolder.folderId)
                setGeneralFolder(newFolder);
        } catch (error: any) {
            toast.error(
                error.message ||
    ...
    const handleDeleteLink = async () => {
        try {
            ...
            toast.success(
                'Link ' + selectedLink.title + ' deleted successfully!',
            );
            const newFolder = {
                ...selectedFolder,
                links: selectedFolder.links.filter(
                    (link: any) => link.linkId !== selectedLink.linkId,
                ),
            };
            setSelectedFolder(newFolder);
            if (selectedFolder.folderId === generalFolder.folderId)
    ...
    const handleCreateEvent = async () => {
        try {
            ...
            toast.success(
                'Event ' + eventTitleInput + ' created successfully!',
            );
            setEvents([...events, newEvent]);

            if (eventReminderInput) {
                const eventDate = getDate(
                    eventWeekInput as any,
                    eventDayInput as any,
                );
                const remindAt = new Date(
                    new Date(eventDate).getTime() - 24 * 60 * 60 * 1000,
                );
                await createReminder(newEvent.eventId, remindAt);
            }
        } catch (error: any) {
            toast.error(
                error.message ||
    ...
    const handleUpdateEvent = async () => {
        try {
            ...
            toast.success(
                'Event ' + eventTitleInput + ' updated successfully!',
            );
            setEvents(
                events.map((event) =>
    ...
    const getAllTasks = async (courseId: string) => {
        try {
            ...
            setWeeklyTasks(
                tasks.filter((task: any) => task.taskType === 'WEEKLY'),
            );
            setTodayTasks(
                tasks.filter((task: any) => task.taskType === 'TODAY'),
            );
        } catch (error: any) {
            toast.error(
                error.message ||
    ...
    const toggleTask = async (taskId: string) => {
        try {
            ...
            toast.error(
                'Failed to update task completion status. Please try again later.',
            );
        }
    };
    ...
    const handleUpdateTask = async (taskId: string, newName: string) => {
        try {
            ...
            setTodayTasks(
                todayTasks.map((task) =>
                    task.taskId === taskId ? { ...task, name: newName } : task,
                ),
            );
            setWeeklyTasks(
                weeklyTasks.map((task) =>
    ...
    const handleDeleteTask = async (taskId: string) => {
        try {
            ...
            setWeeklyTasks(
                weeklyTasks.filter((task) => task.taskId !== taskId),
            );
        ...
        <div className="flex min-h-screen">
            <main className="flex min-w-0 flex-1 flex-col gap-4 ml-6 mt-6">
                            ...
                            <div className="mt-2 font-sans text-muted-foreground">
                                <p>
                                    ...
                                    {formatToMonthDayYear(
                                        courseData?.semesterData[1].examDate,
                                    )}
                                </p>
                            </div>
                                            ...
                                            >
                                                <HoverCardTrigger>
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${(component.hours / 14) * workloadBarLength}px`,
                                                            height: '20px',
                                                        }}
                                                    ></div>
                                                </HoverCardTrigger>
                ...
                <section>
                    ...
                    <div className="mt-4 flex flex-row items-center gap-4">
                        <Button
                            ...
                            className="font-sans px-2 py-1 rounded-md bg-white text-black border hover:bg-primary hover:text-white shadow-sm transition-colors"
                        >
                        ...
                        <Button
                            onClick={() =>
                                setFolderGridMode(
                                    switchMode(folderGridMode, 'EDIT'),
                                )
                            }
                            className={`flex items-center justify-center font-sans p-2 rounded-md ${modeStyle(folderGridMode, 'EDIT')} border shadow-sm transition-colors`}
                        >
                        ...
                        <Button
                            onClick={() =>
                                setFolderGridMode(
                                    switchMode(folderGridMode, 'DELETE'),
                                )
                            }
                            className={`flex items-center justify-center font-sans p-2 rounded-md ${modeStyle(folderGridMode, 'DELETE')} border shadow-sm transition-colors`}
                        >
                ...
                <section className="mt-10">
                    ...
                    <div className="mt-4 flex flex-row items-center gap-4">
                        ...
                        <Button
                            onClick={() =>
                                setLinkGridMode(
                                    switchMode(linkGridMode, 'EDIT'),
                                )
                            }
                            className={`flex items-center justify-center font-sans p-2 rounded-md ${modeStyle(linkGridMode, 'EDIT')} border shadow-sm transition-colors`}
                        >
                            <SquarePen className="w-4 h-4" />
                        </Button>
                        ...
                        <Button
                            onClick={() =>
                                setLinkGridMode(
                                    switchMode(linkGridMode, 'DELETE'),
                                )
                            }
                            className={`flex items-center justify-center font-sans p-2 rounded-md ${modeStyle(linkGridMode, 'DELETE')} border shadow-sm transition-colors`}
                        >
                            <Trash2 className="w-4 h-4" />
                        </Button>
                    </div>
                ...
                <AddEventDialog
                    ...
                    eventReminderInput={eventReminderInput}
                    setEventReminderInput={setEventReminderInput}
