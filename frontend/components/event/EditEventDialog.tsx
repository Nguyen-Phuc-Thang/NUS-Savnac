...
export default function EditEventDialog({
    ...
    useEffect(() => {
        ...
        setEventTypeInput(event?.type);
        setEventWeekInput(event?.week);
        setEventDayInput(event?.day);
        setEventStartTimeHourInput(event?.startTime.split(':')[0]);
        setEventStartTimeMinuteInput(event?.startTime.split(':')[1]);
        setEventEndTimeHourInput(event?.endTime.split(':')[0]);
        setEventEndTimeMinuteInput(event?.endTime.split(':')[1]);
        setEventVenueInput(event?.venue);
    }, [open]);
