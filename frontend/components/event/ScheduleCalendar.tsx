...
export default function ScheduleCalendar({
    ...
    const handleWeekChange = (newWeek: string) => {
        ...
        setEventsToDisplay(
            events
                .filter((event) => event.week === newWeek)
                .map((event) => modifyEvent(event)),
        );
    };
        ...
        <div>
            <div className="mt-5 font-sans flex flex-row items-center">
                <Combobox
                    items={weeks}
                    value={currentWeek}
                    onValueChange={(value) => handleWeekChange(value ?? '')}
                >
                ...
                <Button
                    className="ml-5 bg-white text-black border shadow-sm transition-colors hover:bg-white"
                    onClick={handlePrevWeek}
                >
                    <ChevronLeft />
                </Button>
                <Button
                    className="ml-2 bg-white text-black border shadow-sm transition-colors hover:bg-white"
            ...
            <div className="mt-5 text-sm font-sans italic text-gray-500">
                ...
                <FullCalendar
                    ...
                    eventContent={(info) => (
                        <div className="overflow-hidden text-xs">
                            ...
                            <div>{info.event.extendedProps.course}</div>
                            <div>{info.event.extendedProps.venue}</div>
                        </div>
                    )}
                />
            </div>
