...
export default function FolderPanel({
    ...
    return (
        <aside
            aria-label="Folder panel"
            className={`shrink-0 border-l overflow-hidden transition-all duration-300 ${open ? 'w-[20rem]' : 'w-0'}`}
        >
            {folder && (
                    ...
                    <div className="mt-6 flex flex-col">
                        ...
                        <div className="mt-4 flex flex-row gap-2">
                            <Button
                                onClick={onAdd}
                                aria-label="Add link"
                                className="p-2 rounded-md bg-white text-black border hover:bg-primary hover:text-white shadow-sm transition-colors"
                            >
                        ...
                        <div className="h-50 overflow-y-auto mt-4 flex flex-col gap-3">
                            {folder.links?.map((link: any) => (
                                <div
                                    key={link.linkId}
                                    className="group relative flex items-center rounded-md border border-transparent pr-16 transition-colors hover:border-border hover:bg-muted/50"
                                >
                                    <a
                                        href={link.url}
                                        target="_blank"
