...
export default function FolderGrid({ folders, selectedFolder, setSelectedFolder, mode, onFolderClick, onEditModeFolderClick, onDeleteModeFolderClick }: FolderGridProps) {
    ...
    return (
        <div className="mt-5">
            <p className='text-sm text-muted-foreground italic'>
                {mode === "EDIT" && "You are in Edit mode. Click on a folder to edit its details."}
                {mode === "DELETE" && "You are in Delete mode. Click on a folder to delete it."}
            </p>
                    ...
                    folders.map((folder) => (
                        <div className='flex flex-col items-center' key={folder.folderId}>
                            {folder.folderId == selectedFolder?.folderId
                                ? <FolderOpen onClick={() => handleFolderClick(folder)} size={120} />
                                : <FolderClosed onClick={() => handleFolderClick(folder)} size={120} />}
